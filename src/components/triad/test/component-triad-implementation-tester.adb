--------------------------------------------------------------------------------
-- Triad Component Tester Body
--------------------------------------------------------------------------------

-- Includes:
with Parameter;

package body Component.Triad.Implementation.Tester is
   ...
   begin
      ...
      if Id_To_Return = 0 then
         case Arg.Id is
            -- ID for Sun_Direction_Inertial:
            when 0 => Id_To_Return := 0;
            -- ID for Thrust_Direction_Body:
      ...
      if Length_To_Return = 0 then
         case Arg.Id is
            -- Length for Sun_Direction_Inertial:
            when 0 => Length_To_Return := Packed_F32x3.Size_In_Bytes;
            -- Length for Thrust_Direction_Body:
         ...
         case Arg.Id is
            ...
            when 0 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_F32x3.Size_In_Bytes - 1) :=
                  Packed_F32x3.Serialization.To_Byte_Array (Self.Sun_Direction_Inertial);
            -- Length for Thrust_Direction_Body:
            when 1 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Packed_F32x3.Size_In_Bytes - 1) :=
                  Packed_F32x3.Serialization.To_Byte_Array (Self.Thrust_Direction_Body);
