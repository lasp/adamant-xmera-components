--------------------------------------------------------------------------------
-- Rw_Motor_Torque Component Tester Body
--------------------------------------------------------------------------------

-- Includes:
with Parameter;

package body Component.Rw_Motor_Torque.Implementation.Tester is
   ...
   begin
      ...
      Self.Rwa_Torques_T_Recv_Sync_History.Init (Depth => 100);
   end Init_Base;
   ...
   begin
      ...
      Self.Rwa_Torques_T_Recv_Sync_History.Destroy;
   end Final_Base;
   ...
   begin
      ...
      Self.Component_Instance.Attach_Rwa_Torques_T_Send (To_Component => Self'Unchecked_Access, Hook => Self.Rwa_Torques_T_Recv_Sync_Access);
         ...
         case Arg.Id is
            ...
            when 0 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Cmd_Torque_Body.Size_In_Bytes - 1) :=
                  Cmd_Torque_Body.Serialization.To_Byte_Array (Self.Control_Torque);
            -- Length for Wheel_Speeds:
            when 1 =>
   ...
   -- Send the computed per-wheel motor torques directly to the wheel interface
   -- component. Only honored by the receiver while the wheels are in closed-loop
   -- control.
   overriding procedure Rwa_Torques_T_Recv_Sync (Self : in out Instance; Arg : in Rwa_Torques.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Rwa_Torques_T_Recv_Sync_History.Push (Arg);
   end Rwa_Torques_T_Recv_Sync;
...
end Component.Rw_Motor_Torque.Implementation.Tester;
