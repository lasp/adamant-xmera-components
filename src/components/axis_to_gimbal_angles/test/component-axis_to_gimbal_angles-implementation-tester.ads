--------------------------------------------------------------------------------
-- Axis_To_Gimbal_Angles Component Tester Spec
--------------------------------------------------------------------------------

-- Includes:
with Component.Axis_To_Gimbal_Angles_Reciprocal;
...
with Packed_F32x3;
with Data_Product;
with Axis_To_Gimbal_Angles_Output.Representation;

-- Axis to gimbal angles. Maps a commanded body frame thrust direction onto the
-- two gimbal angles that place the gimbal thrust axis on it. A request beyond the
-- travel of the mechanism is pulled back onto the cone of the largest deflection,
-- and a request with no direction leaves the gimbal at neutral. Wraps the
-- AxisToGimbalAnglesAlgorithm C++ algorithm via its C shim.
package Component.Axis_To_Gimbal_Angles.Implementation.Tester is

   use Component.Axis_To_Gimbal_Angles_Reciprocal;
   ...
   package Gimbal_Command_History_Package is new Printable_History (Axis_To_Gimbal_Angles_Output.T, Axis_To_Gimbal_Angles_Output.Representation.Image);

   -- Component class instance:
   type Instance is new Component.Axis_To_Gimbal_Angles_Reciprocal.Base_Instance with record
      -- The component instance under test:
      Component_Instance : aliased Component.Axis_To_Gimbal_Angles.Implementation.Instance;
      -- Connector histories:
      Data_Product_Fetch_T_Service_History : Data_Product_Fetch_T_Service_History_Package.Instance;
      Data_Product_T_Recv_Sync_History : Data_Product_T_Recv_Sync_History_Package.Instance;
      -- Data product histories:
      Gimbal_Command_History : Gimbal_Command_History_Package.Instance;
   ...
   -----------------------------------------------
   -- Description:
   --    Data products for the Axis To Gimbal Angles component.
   -- The two gimbal angles that align the gimbal thrust axis with the commanded
   -- direction, and the unit thrust direction those angles achieve in body frame
   -- components.
   overriding procedure Gimbal_Command (Self : in out Instance; Arg : in Axis_To_Gimbal_Angles_Output.T);
...
end Component.Axis_To_Gimbal_Angles.Implementation.Tester;
