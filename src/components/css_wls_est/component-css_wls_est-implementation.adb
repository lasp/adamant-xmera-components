...
package body Component.Css_Wls_Est.Implementation is
   ...
   -- boresight parameter carries the three components of each sensor in row major
   -- order, while the C record interleaves each sensor's boresight with its bias.
   function To_Constellation (
      ...
      Result : Css_Wls_Est_Constellation.C.U_C := (
         ...
         Css_Sensors => [others => (N_Hat_B => [others => 0.0], Bias => 0.0)]
      );
   begin
      for Sensor in 0 .. Num_Sensors - 1 loop
         for Axis in 0 .. 2 loop
            Result.Css_Sensors (Sensor).N_Hat_B (Axis) := Css_N_Hat_B (Sensor * 3 + Axis);
         end loop;
         Result.Css_Sensors (Sensor).Bias := Css_Bias (Sensor);
      end loop;
      return Result;
   ...
   begin
      ...
      -- Call the algorithm:
      declare
         -- The measurements cross by reference, one entry per sensor.
      ...
      begin
         ...
         begin
            ...
            Self.Data_Product_T_Send (Self.Data_Products.Post_Fit_Residuals (
               Arg.Time, Packed_F32x8.C.Pack (Output.Post_Fit_Residuals)));
         end;
      end;
