...
package body Component.Body_Rate_Miscompare.Implementation is
   ...
   begin
      ...
      declare
         ...
         Output : constant Body_Rate_Miscompare_Output.T := Body_Rate_Miscompare_Output.C.Pack (Update (
            Self.Alg,
            Imu_Omega => Imu_Omega,
            St_Omega  => St_Omega
         ));
      begin
         ...
         Self.Data_Product_T_Send (Self.Data_Products.Rate_Fault_Status (Arg.Time, (Fault_Detected => Output.Body_Rate_Fault_Detected)));
         ...
         if Output.Body_Rate_Fault_Detected and then not Self.Prev_Fault_Latched then
         ...
         elsif not Output.Body_Rate_Fault_Detected and then Self.Prev_Fault_Latched then
            Self.Event_T_Send_If_Connected (Self.Events.Body_Rate_Fault_Cleared (Arg.Time));
         end if;
         Self.Prev_Fault_Latched := Output.Body_Rate_Fault_Detected;
      end;
