--------------------------------------------------------------------------------
-- Thrust_Vectoring Component Tester Body
--------------------------------------------------------------------------------

-- Includes:
with Parameter;

package body Component.Thrust_Vectoring.Implementation.Tester is
   ...
   begin
      ...
      Self.Thrust_Direction_History.Init (Depth => 100);
   end Init_Base;
   ...
   begin
      ...
      -- Data product histories:
      Self.Thrust_Direction_History.Destroy;
   end Final_Base;
         ...
         case Arg.Id is
            ...
            when 0 =>
               Buffer_To_Return (Buffer_To_Return'First .. Buffer_To_Return'First + Cmd_Torque_Body.Size_In_Bytes - 1) :=
                  Cmd_Torque_Body.Serialization.To_Byte_Array (Self.Torque_Request);
   ...
   overriding procedure Thrust_Direction (Self : in out Instance; Arg : in Packed_F32x3.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Thrust_Direction_History.Push (Arg);
   end Thrust_Direction;
