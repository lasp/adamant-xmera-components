--------------------------------------------------------------------------------
-- Mrp_Feedback Component Implementation Body
--------------------------------------------------------------------------------
...
package body Component.Mrp_Feedback.Implementation is
   ...
   type Pointer_Config is record
      ...
      Rw_Config : aliased Mrp_Feedback_Rw_Config.C.U_C;
   end record;

   -- Marshal the pointer arguments of the configuration. The wheel count is a hardware
   -- fact fixed by the width of the wheel speed and configuration types, so it is
   -- derived here rather than parameterized.
   function To_Pointer_Config (
      ...
      (Known_Torque => (Value => Packed_F32x3.C.To_C (Known_Torque_Pnt_B_B)),
       ...
       Rw_Config => (Num_Rw => Unsigned_32 (Packed_F32x4.Length),
                     Gs_Matrix_B => Packed_F32x3_X4.C.To_C (Rw_Spin_Axes),
                     Js_List => Packed_F32x4.C.To_C (Rw_Inertias),
                     Wheel_Availability => [for I in Int32_X4.C.U_C'Range => To_C (Wheel_Availability (I))]));

   --------------------------------------------------
   -- Subprogram for implementation init method:
   --------------------------------------------------
   -- Initializes the MRP feedback algorithm with the control period and the default
   -- parameter values.
   ...
   begin
      ...
      Self.Alg := Create (
         ...
         Rw_Configuration     => Cfg.Rw_Config'Access);
   end Init;
   ...
   begin
      ...
      -- Call the C algorithm and publish both torques. Update is qualified because
      -- Parameter_Enums also declares one.
      declare
         Output : constant Mrp_Feedback_Output.C.U_C := Mrp_Feedback_Algorithm_C.Update (
            Self.Alg,
            Att_Guid_Input => Guidance_C'Access,
            Wheel_Speeds   => Speeds_C'Access);
      begin
         Self.Data_Product_T_Send (Self.Data_Products.Control_Torque (
            Arg.Time,
            (Torque_Request_Body => Packed_F32x3.C.Pack (Output.Control_Torque))
         ));
         Self.Data_Product_T_Send (Self.Data_Products.Integral_Feedback_Torque (
            Arg.Time,
            (Torque_Request_Body => Packed_F32x3.C.Pack (Output.Integral_Feedback_Torque))
         ));
      end;
   end Tick_T_Recv_Sync;

   -- Zero the integral of the attitude error. Called on GNC state change so the
   ...
   -----------------------------------------------
   -- Description:
   --    Parameters for the Mrp Feedback component.
   ...
   overriding function Validate_Parameters (
      ...
      Control_Law_Type : in Packed_Control_Law_Type.U;
      Known_Torque_Pnt_B_B : in Packed_F32x3.U;
      Inertia : in Packed_F32x9.U;
      Rw_Spin_Axes : in Packed_F32x3_X4.U;
      Rw_Inertias : in Packed_F32x4.U;
   ...
   -----------------------------------------------
   -- Description:
   --    Data dependencies for the Mrp Feedback component.
...
end Component.Mrp_Feedback.Implementation;
