--------------------------------------------------------------------------------
-- Mrp_Feedback Component Tester Spec
--------------------------------------------------------------------------------

-- Includes:
with Component.Mrp_Feedback_Reciprocal;
...
-- MRP feedback attitude control law with integral feedback and reaction wheel
-- momentum compensation. Computes the commanded body torque from the attitude and
-- rate tracking errors, the reference motion, and the reaction wheel speeds.
-- Wraps the MrpFeedbackAlgorithm C++ algorithm via its C shim.
package Component.Mrp_Feedback.Implementation.Tester is

   use Component.Mrp_Feedback_Reciprocal;
   ...
   package Control_Torque_History_Package is new Printable_History (Cmd_Torque_Body.T, Cmd_Torque_Body.Representation.Image);
   package Integral_Feedback_Torque_History_Package is new Printable_History (Cmd_Torque_Body.T, Cmd_Torque_Body.Representation.Image);

   -- Component class instance:
   type Instance is new Component.Mrp_Feedback_Reciprocal.Base_Instance with record
      -- The component instance under test:
      Component_Instance : aliased Component.Mrp_Feedback.Implementation.Instance;
      -- Connector histories:
      Data_Product_Fetch_T_Service_History : Data_Product_Fetch_T_Service_History_Package.Instance;
      Data_Product_T_Recv_Sync_History : Data_Product_T_Recv_Sync_History_Package.Instance;
      -- Data product histories:
      Control_Torque_History : Control_Torque_History_Package.Instance;
      Integral_Feedback_Torque_History : Integral_Feedback_Torque_History_Package.Instance;
   ...
   -----------------------------------------------
   -- Description:
   --    Data products for the Mrp Feedback component.
   -- [Nm] Commanded control torque in body frame components.
   overriding procedure Control_Torque (Self : in out Instance; Arg : in Cmd_Torque_Body.T);
   -- [Nm] The integral feedback part of the commanded torque, in body frame
   -- components. Zero when the integral gain is zero.
   overriding procedure Integral_Feedback_Torque (Self : in out Instance; Arg : in Cmd_Torque_Body.T);
...
end Component.Mrp_Feedback.Implementation.Tester;
