--------------------------------------------------------------------------------
-- Mrp_Feedback Component Tester Body
--------------------------------------------------------------------------------

-- Includes:
with Parameter;

package body Component.Mrp_Feedback.Implementation.Tester is
   ...
   begin
      ...
      Self.Control_Torque_History.Init (Depth => 100);
      Self.Integral_Feedback_Torque_History.Init (Depth => 100);
   end Init_Base;
   ...
   begin
      ...
      -- Data product histories:
      Self.Control_Torque_History.Destroy;
      Self.Integral_Feedback_Torque_History.Destroy;
   end Final_Base;
   ...
   overriding procedure Control_Torque (Self : in out Instance; Arg : in Cmd_Torque_Body.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Control_Torque_History.Push (Arg);
   end Control_Torque;

   -- [Nm] The integral feedback part of the commanded torque, in body frame
   -- components. Zero when the integral gain is zero.
   overriding procedure Integral_Feedback_Torque (Self : in out Instance; Arg : in Cmd_Torque_Body.T) is
   begin
      -- Push the argument onto the test history for looking at later:
      Self.Integral_Feedback_Torque_History.Push (Arg);
   end Integral_Feedback_Torque;
